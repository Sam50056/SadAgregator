        ...
        case 3:
            ...
            if item.isSwitch{
                ...
                if item.labelText == "Без замен"{
                    ...
                    infoButton.addAction(UIAction(handler: { [weak self] _ in
                        self?.showSimpleAlertWithOkButton(title: "Проверка на брак", message: "Включая эту опцию для выбранного товара вы передаете информацию посреднику о том, что желаете проверить товар на брак. Однако учитывайте, это платная услуга и у всех посредников на нее своя дополнительная цена. Если выбранный посредник не оказывает эту услугу, вы не сможете передать ему закупку с таким товаром. Помимо стоимости услуг посредника будет добавлена еще стоимость проверки на брак")
                    }), for: .touchUpInside)
